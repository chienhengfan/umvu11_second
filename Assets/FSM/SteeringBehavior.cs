...
public class SteeringBehavior
{
    ...
    static public bool CollisionAvoid(AIData data)
    {
        ...
        /*
        List<ColTest5> enemies = Main.m_Instance.GetEnemies();
        iCount = enemies.Count;
        for (int i = 0; i < iCount; i++) { 
        
            if (enemies[i].gameObject == data.m_Go)
            {
                continue;
            }
            vec = enemies[i].transform.position - cPos;
            vec.y = 0.0f;
            fDist = vec.magnitude;
            float fR = enemies[i].m_Data.m_fRadius;
            if (fDist > data.m_fProbeLength + fR)
            {
               // m_AvoidTargets[i].m_eState = Obstacle.eState.OUTSIDE_TEST;
                continue;
            }
