...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        [Header("Attack Tools")]
        [Tooltip("The Major weapon that can attack from near to far")]
        ...
        private int _animIDQuickAttack01;
        private int _animIDTakeBow;
        ...
        private int quickBowNum = 0;
        private float bowIdleTime = 2.5f;
        private float currentIdleTime = 0.0f;
        private bool isInterval = false;

        private GameObject[] arrowPools;
        ...
        private void Start()
        {
            ...
            camOriPos = CinemachineCameraTarget.gameObject.transform.localPosition;
#if ENABLE_INPUT_SYSTEM
        ...
        {
            float curTime = 0.0f;
            while (isInterval == true)
            {
                Debug.Log("Player curTime: " + curTime);
                curTime += Time.deltaTime;
                if (curTime <= interval) { continue; }
                break;
            }
            isInterval = false;
        }
        ...
        //{
        //    arrowPools = new GameObject[10];
        //    for (int i = 0; i < arrowPools.Length; i++)
        //    {
        //        arrowPools[i] = Instantiate(iceArrow, transform.position, Quaternion.identity);
        //        arrowPools[i].SetActive(false);
        //    }
        //}

        public void WeaponSwitch()
        {
            Debug.Log("Unsheath bow and Fight");
            bowObject.SetActive(true);
            Debug.Log("Bow Active");
        }
        public void WeaponSwitch02()
        {
            bowObject.SetActive(false);
            mState = motionState.Idle;
            Debug.Log("Bow InActive");
        }

        private void DieCheckAndDo()
        {
            if (playerHP <= 0)
            {
        ...
        private void AimBowAttack()
        {
            ...
            {
                mState = motionState.Aim;
                CinemachineCameraTarget.transform.localPosition = camOriPos + Camera.main.transform.right *1;

                //Vector3 vTo = CinemachineCameraTarget.transform.position - this.transform.position;
                //vTo.Normalize();
                //transform.forward = Vector3.Lerp(transform.forward, vTo, 0.1f);
            ...
            else
            {
                mState = motionState.Idle;
                _animator.SetBool("IsAim", false);
                CinemachineCameraTarget.transform.localPosition = camOriPos;
            }

            if(mState == motionState.Aim)
            {
                Debug.Log("Aim State No Move");
                _animator.SetBool("IsAim", true);
                ...
                else if (mouseY <= -0.4f) { mouseY = -0.4f; }
                _animator.SetFloat("mouseHorizontal", mouseX);
                _animator.SetFloat("mouseHorizontal", mouseY);
            }
        }
        ...
        private void AssignAnimationIDs()
        {
            ...
            _animIDQuickAttack01 = Animator.StringToHash("QuickAttack01");
            _animIDTakeBow = Animator.StringToHash("TakeBow");
        }
        ...
        IEnumerator Run(float sp)
        {
            _animator.SetFloat(_animIDSpeed, sp);
